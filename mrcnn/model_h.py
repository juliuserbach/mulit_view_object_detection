...
def get_angles(pos, i, d_model):
    angle_rates = 1 / tf.math.pow(10000, (2 * (i//2)) / tf.float32(d_model))
    return pos * angle_rates


def positional_encoding(position, d_model):
    angle_rads = get_angles(position[..., tf.newaxis],
...
class Transformer(KM.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, 
        ...
        self.final_layer = KL.Dense(target_vocab_size)
...
#         x = KL.Lambda(lambda x: tf.transpose(x, [0, 5, 2, 3, 4, 1]))(x)
        ...
        x = KL.Activation('relu')(x)
#         x = KL.Lambda(lambda x: tf.concat([x[:,0,:,:,:,:], x[:,1,:,:,:,:]], axis=-1))(x)
        print("shape before 3dconv: {}".format(x.get_shape().as_list()))
...
#         x = KL.LeakyReLU(alpha=0.01)(x)
        ...
        print("shape before lstm3d: {}".format(x.get_shape().as_list()))
        x = convlstm(x, name=name_conv, kernel=[3, 3, 3], filters=64)
        print("lstm3d succ")
        print("shape after lstm3d: {}".format(x.get_shape().as_list()))
        x = add_bn_layer(name=name_bn)(x, training=config.TRAIN_BN)
        x = KL.Activation('relu')(x)
...
#             print("image shape in generator: {}".format(image[0].shape))
            ...
            if b == 0:
                ...
                batch_images = np.zeros(
                    (batch_size, config.NUM_VIEWS,) + image[0].shape, dtype=np.float32)
                ...
                batch_gt_R = np.zeros(
                    (batch_size, config.NUM_VIEWS, 3, 4), dtype=np.float32)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        print("input_image_0_shape: {}".format(input_image_0.get_shape().as_list()))
        
#         if 'resnet' not in reused_resnet_lay:
#             reused_resnet_lay['resnet'] = bb_resnet_fpn(name="bb_resnet_fpn",config=config, architecture=config.BACKBONE, stage5=True, train_bn=True)
        
#         fn = lambda input_image_t: build_resnet_fpn(input_image_t, config=config)
#         out = KL.Lambda(lambda x: K.map_fn(fn, x, dtype=[x.dtype, x.dtype, x.dtype, x.dtype, x.dtype]), name="backbone_map_fn")(input_image_t)
#         build_resnet_fpn_layer = KL.Lambda(lambda input_image_t: build_resnet_fpn(input_image_t, config=config))
#         out = KL.TimeDistributed(build_resnet_fpn_layer)(input_image_t)

        backbone_model = view_merger_model(input_image_0.get_shape().as_list()[1:], config)
#         P2, P3, P4, P5, P6 = KL.TimeDistributed(backbone_model)(input_image)
