...
#                     if i > 0:
#                         if np.linalg.norm(Rcam[0][:,3] - R[:,3]) > 0.2:
#                             print("abstand: {}".format(np.linalg.norm(Rcam[0][:,3] - R[:,3])))
#                             continue
#                         if np.linalg.norm(get_delta_rot(Rcam[0][:,:2], R[:,:2])) > 0.1744: #smaller than ~10 degrees
#                             print("angle: {}".format(np.linalg.norm(get_delta_rot(Rcam[0][:,:2], R[:,:2]))))
#                             continue
#                     print("abstand: {}".format(np.linalg.norm(Rcam[0][:,3] - R[:,3])))
#                     print("angle: {}".format(np.linalg.norm(get_delta_rot(Rcam[0][:,:2], R[:,:2]))))
                    Rcam.append(R)
                ...
                for i in range(actual_num_views):
                    ...
                    R = dataset.load_R(image_ids[i])
#                     if i > 0:
#                         if np.linalg.norm(Rcam[0][:,3] - R[:,3]) > 1:
#                             print("abstand: {}".format(np.linalg.norm(Rcam[0][:,3] - R[:,3])))
#                             continue
#                         if np.linalg.norm(get_delta_rot(Rcam[0][:,:2], R[:,:2])) > 0.5: #smaller than ~10 degrees
#                             print("angle: {}".format(np.linalg.norm(get_delta_rot(Rcam[0][:,:2], R[:,:2]))))
#                             continue
                    Rcam.append(R)
                Kmat = dataset.K
