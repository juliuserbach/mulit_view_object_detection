...
#                 mrcnn_bbox = KL.Lambda(lambda x: x*1.0)(mrcnn_bbox)
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors, input_R, input_Kmat, input_depths],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 mrcnn_mask, rpn_rois, rpn_class, rpn_bbox, P2, P5],
                             name='mask_rcnn')
...
#             clipnorm=self.config.GRADIENT_CLIP_NORM)
    ...
    def detect(self, images, Rcam, Kmat, depth, verbose=0):
        ...
        if verbose:
            ...
            log("Rcam", Rcam)
            log("Kmat", Kmat)
            log("depth images", depth)
        # Run object detection
        detections, _, _, mrcnn_mask, _, _, _, mrcnn_features, mrcnn_intermediate_features =\
            self.keras_model.predict([molded_images, image_metas, anchors, Rcam, Kmat, depth], verbose=0)
        # Process detections
        results = []
