...
#     Rcam = repeat_tensor(Rcam[:,0,:,:], config.NUM_VIEWS, rep_dim=1)
#     print("Rcam: {}:".format(Rcam.get_shape().as_list()))
    
    
#     bs, im_bs, h, w, d, ch = grid.get_shape().as_list()
#     Kmat = repeat_tensor(Kmat, im_bs, rep_dim=1)
...
#             rs_grid = tf.tile(rs_grid, [bs, im_bs, 1, 1])#[bs, im_bs, 1, 1]
...
#             Xc = tf.concat(
#                 [Xc, tf.ones([bs, im_bs, 1, npix])],#[bs, im_bs, 1, 1]
#                 axis=-2)
#             Xc = Xc * 0.935
...
#             lshape = [bs * im_bs] + tf_static_shape(sample_locs)[1:] #tf_static_shape(sample_locs) [bs * 1]
...
#                 bs, im_bs, proj_size, proj_size, -1    #bs, im_bs, proj_size, proj_size, -1
#             ])
